...
static void (*timeCallback)(long currentTime, long duration);
...
//        LOGE("播放 %d ",audio->queue.size());
        timeCallback(audio->clock * 1000000, audio->duration);
    } else
        LOGE("解码错误");
}
...
void FFmpegAudio::setAvCodecContext(AVCodecContext *codecContext, long duration) {
    codec = codecContext;
...
void FFmpegAudio::setAudioCallback(void (*call)(long, long)) {
    timeCallback = call;
}
